                                ...
                                <option value="0">...</option>
                                  <?php
                                    ...
                                    foreach ($cats as $cat) {
                                      ...
                                      $childern =getAll("*", "categories", "WHERE parent = {$cat['ID']}", "", "ID");
		  ...
		  <?php
            ...
            echo "<div class='container' >";
              ...
              if (empty($formErrors)) {
                ...
                // insert the data in database
                $stmt = $con->prepare("INSERT INTO items(name, description, price, country_made, status, add_date, cat_ID, member_ID, tags) VALUES(:NAME, :DESCR, :PRICE, :COUNTRY, :STATUS, now(), :CAT, :MEMBER, :TAGS)");
                $stmt->execute(array(
                  'NAME'    => $name,
                  'DESCR'   => $desc, 
                  'PRICE'   => $price,
          ...
          <div class="container">
            <form class="form-horizontal" action="?do=Update" method="POST">
              ...
              <div class="form-group form-group-lg">
                  <label class="control-label col-sm-2">Tags</label>
                  <div class="col-sm-10">
                      <input type="text" name="tags" class="form-control" value="<?php echo $item['tags'] ?>" placeholder="separete Tags with coma (,)">
                  </div>
              </div>
              <!--submit-->
              <div class="form-group form-group-lg">
      ...
      echo "<div class='container' >";
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($formErrors)) {
              ...
              $stmt = $con->prepare("UPDATE items SET name = ?, description = ?, price = ?, country_made = ?, status = ?, member_ID = ?, cat_ID = ?, tags = ? WHERE itemID = ?");
